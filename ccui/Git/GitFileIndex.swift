...
nonisolated struct GitFileIndex: Sendable {
    ...
    nonisolated static func build(repositoryPath: String) async -> GitFileIndex {
        async let nonIgnoredTask = Task.detached(priority: .userInitiated) {
            await nonIgnoredFiles(at: repositoryPath)
        }.value
        async let ignoredDirsTask = Task.detached(priority: .userInitiated) {
            await ignoredDirectories(at: repositoryPath)
        }.value
        async let ignoredFilesTask = Task.detached(priority: .userInitiated) {
            await ignoredIndividualFiles(at: repositoryPath)
        }.value
    ...
    private static func nonIgnoredFiles(at repositoryPath: String) async -> Set<String> {
        let output: String
        do {
            output = try await GitClient.lsFiles(["--cached", "--others", "--exclude-standard", "-z"], at: repositoryPath)
        } catch {
    ...
    private static func ignoredDirectories(at repositoryPath: String) async -> (files: Set<String>, dirPrefixes: [String]) {
        let output: String
        do {
            output = try await GitClient.lsFiles(["--others", "--ignored", "--exclude-standard", "--directory", "-z"], at: repositoryPath)
        } catch {
    ...
    private static func ignoredIndividualFiles(at repositoryPath: String) async -> Set<String> {
        let output: String
        do {
            output = try await GitClient.lsFiles(["--others", "--ignored", "--exclude-standard", "-z"], at: repositoryPath)
        } catch {
